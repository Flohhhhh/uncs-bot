...
Inspect the generated handler, replace its placeholder response, and extend it with Necord option decorators when needed. For subcommands or interactive components, follow the patterns in the task-specific skill. Verify with `npm run lint`, `npm run build`, and `git diff --check` when dependencies are installed.
